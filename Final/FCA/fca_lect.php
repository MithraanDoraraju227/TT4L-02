...
$comments = [];
$sql = "SELECT rating, comment FROM comments WHERE topic = 'FCA' AND sector = 'Lecturers' ";
...
$averageRating = 0;
$sql = "SELECT AVG(rating) AS avg_rating FROM comments WHERE topic = 'FCA' AND sector ='Lecturers' ";
$result = $conn->query($sql);
...
<body>
    ...
    <ul>
        <div class="icon">
            <h2 class="logo">FCA Lecturers</h2>
        </div>
    ...
    <h2><class="average-rating" style="color: white;">
        <?php if ($averageRating): ?>
            Average Rating FCA - Lecturers: <?php echo number_format($averageRating, 2); ?> / 5
        <?php else: ?>
            No ratings available for FCA - Lecturers.
        <?php endif; ?>
        </h2></div>  
