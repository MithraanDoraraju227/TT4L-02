...
$comments = [];
$sql = "SELECT rating, comment FROM comments WHERE topic = 'FCA' AND sector = 'Toilet' ";
...
$averageRating = 0;
$sql = "SELECT AVG(rating) AS avg_rating FROM comments WHERE topic = 'FCA' AND sector ='Toilet' ";
$result = $conn->query($sql);
...
<body>
    ...
    <h2><class="average-rating" style="color: white;">
        <?php if ($averageRating): ?>
            Average Rating FCA - Toilets: <?php echo number_format($averageRating, 2); ?> / 5
        <?php else: ?>
            No ratings available for FCA - Toilets.
        <?php endif; ?>
        </h2></div>  
