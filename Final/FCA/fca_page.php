...
$comments = [];
$sql = "SELECT rating, comment FROM comments WHERE topic = '$selectedTopic'";
...
$averageRating = 0;
$sql = "SELECT AVG(rating) AS avg_rating FROM comments WHERE topic = '$selectedTopic'";
$result = $conn->query($sql);
...
<body>
    ...
    <ul>
        <div class="icon">
            <h2 class="logo">Faculty of Cinematic Arts (FCA)</h2>
        </div>
    ...
    <div class="average-rating" style="color: white;">
        <?php if ($averageRating): ?>
            Average Rating for FCA: <?php echo number_format($averageRating, 2); ?> / 5
        <?php else: ?>
            No ratings available for FCA.
        <?php endif; ?>
    </div>  
